 ...
 *
 * <p>Odpowiada za pelny cykl zycia konta: rejestracja, weryfikacja emaila, logowanie, odswiezanie
...
public class AuthService {
   ...
   *
   * <p>Hashuje haslo BCrypt (cost 12 w konfiguracji PasswordEncoder), tworzy ustawienia domyslne,
   ...
   * @throws AuthException gdy kod jest nieprawidlowy, przeterminowany lub juz uzyty
   */
  @Transactional
   ...
   * @throws AuthException gdy refresh token jest nieprawidlowy lub przeterminowany
   */
  @Transactional
  ...
  /**
   * Inicjuje proces resetu hasla — generuje token i wysyla email z linkiem.
   *
   * <p>Celowo nie rzuca wyjatku gdy email nie istnieje — nie ujawniamy czy konto jest w systemie
   * (ochrona przed user enumeration). Wszystkie poprzednie tokeny resetu dla uzytkownika sa
   * uniewazniane przed wystawieniem nowego.
   *
   * @param request email uzytkownika proszczego o reset
   */
  @Transactional
  public void forgotPassword(ForgotPasswordRequest request) {
    userRepository
        ...
        .ifPresent(
            user -> {
              // Uniewnaznienie poprzednich tokenow — jeden aktywny token na konto
              passwordResetRepository.deleteAllByUserId(user.getId());

              String rawToken = emailService.generateResetToken();
              ...
              PasswordReset resetToken =
                  PasswordReset.builder()
                      .user(user)
                      .tokenHash(tokenHash)
                      .expiresAt(emailService.resetTokenExpiry())
                      .build();
  ...
  /**
   * Ustawia nowe haslo uzytkownika po weryfikacji tokena z emaila.
   *
   * <p>Po udanym resecie uniewnaznia wszystkie refresh tokeny — wymuszamy ponowne logowanie na
   * wszystkich urzadzeniach (bezpieczenstwo przy przejęciu konta).
   *
   * @param request token z emaila i nowe haslo
   * @throws AuthException gdy token jest nieprawidlowy, wygasl lub juz uzyty
   */
  @Transactional
  public void resetPassword(ResetPasswordRequest request) {
    String tokenHash = jwtProvider.hashToken(request.token());

    PasswordReset resetToken =
        passwordResetRepository
            .findByTokenHash(tokenHash)
            .orElseThrow(() -> new AuthException("Invalid or expired password reset link."));

    if (resetToken.isExpired()) {
      throw new AuthException("Password reset link has expired. Please request a new one.");
    }
    if (resetToken.isUsed()) {
      throw new AuthException("Password reset link has already been used.");
    }
    ...
    // Uniewnaznienie wszystkich sesji — bezpieczenstwo przy przejęciu konta
  ...
  /**
   * Loguje lub rejestruje uzytkownika przez Google Sign-In.
   *
   * <p>Weryfikuje Google ID token, a nastepnie: - jesli uzytkownik istnieje — zwraca tokeny sesji,
   * - jesli nie — tworzy nowe konto automatycznie (pelna rejestracja).
   *
   * @param request Google ID token z klienta mobilnego
   * @return tokeny sesji (access + refresh)
   * @throws AuthException gdy Google ID token jest nieprawidlowy
   */
  @Transactional
  public AuthResponse googleLogin(GoogleAuthRequest request) {
    User user = oauthService.authenticateWithGoogle(request.idToken());
    log.info("Google OAuth login for user: {}", user.getEmail());
    return generateAndPersistTokens(user);
  }
