 ...
 *
 * <p>W tej implementacji (Krok 2) uzywamy lokalnej mapy in-memory per instancja JVM. Pozwala to na
 * pelne testy bez Redis. Dla produkcji / wielu instancji nalezy zastapic ConcurrentHashMap przez
 * Bucket4j-Redis (ProxyManager z config.redis.asAsync...) — to zostanie dodane w etapie CI/CD.
 *
 * <p>Klucze: "{endpoint}:{ipAddress}" — granularnosc per endpoint i per klient.
 */
@Service
public class RateLimitService {
  ...
  /**
   * Sprawdza czy IP moze zainicjowac kolejny reset hasla.
   *
   * @param ipAddress adres IP klienta
   * @throws RateLimitExceededException gdy limit zostal przekroczony
   */
